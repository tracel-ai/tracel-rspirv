...
impl Decoder<'_> {
    ...
    pub fn fp_fast_math_mode(&mut self) -> Result<spirv::FPFastMathMode> {
        if let Ok(word) = self.word() {
            spirv::FPFastMathMode::from_bits(word)
                .ok_or(Error::FPFastMathModeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn selection_control(&mut self) -> Result<spirv::SelectionControl> {
        if let Ok(word) = self.word() {
            spirv::SelectionControl::from_bits(word)
                .ok_or(Error::SelectionControlUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn function_control(&mut self) -> Result<spirv::FunctionControl> {
        if let Ok(word) = self.word() {
            spirv::FunctionControl::from_bits(word)
                .ok_or(Error::FunctionControlUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn memory_semantics(&mut self) -> Result<spirv::MemorySemantics> {
        if let Ok(word) = self.word() {
            spirv::MemorySemantics::from_bits(word)
                .ok_or(Error::MemorySemanticsUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn kernel_profiling_info(&mut self) -> Result<spirv::KernelProfilingInfo> {
        if let Ok(word) = self.word() {
            spirv::KernelProfilingInfo::from_bits(word)
                .ok_or(Error::KernelProfilingInfoUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn ray_flags(&mut self) -> Result<spirv::RayFlags> {
        if let Ok(word) = self.word() {
            spirv::RayFlags::from_bits(word).ok_or(Error::RayFlagsUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn fragment_shading_rate(&mut self) -> Result<spirv::FragmentShadingRate> {
        if let Ok(word) = self.word() {
            spirv::FragmentShadingRate::from_bits(word)
                .ok_or(Error::FragmentShadingRateUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn raw_access_chain_operands(&mut self) -> Result<spirv::RawAccessChainOperands> {
        if let Ok(word) = self.word() {
            spirv::RawAccessChainOperands::from_bits(word)
                .ok_or(Error::RawAccessChainOperandsUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn source_language(&mut self) -> Result<spirv::SourceLanguage> {
        if let Ok(word) = self.word() {
            spirv::SourceLanguage::from_u32(word)
                .ok_or(Error::SourceLanguageUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn execution_model(&mut self) -> Result<spirv::ExecutionModel> {
        if let Ok(word) = self.word() {
            spirv::ExecutionModel::from_u32(word)
                .ok_or(Error::ExecutionModelUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn addressing_model(&mut self) -> Result<spirv::AddressingModel> {
        if let Ok(word) = self.word() {
            spirv::AddressingModel::from_u32(word)
                .ok_or(Error::AddressingModelUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn sampler_addressing_mode(&mut self) -> Result<spirv::SamplerAddressingMode> {
        if let Ok(word) = self.word() {
            spirv::SamplerAddressingMode::from_u32(word)
                .ok_or(Error::SamplerAddressingModeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn sampler_filter_mode(&mut self) -> Result<spirv::SamplerFilterMode> {
        if let Ok(word) = self.word() {
            spirv::SamplerFilterMode::from_u32(word)
                .ok_or(Error::SamplerFilterModeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn image_channel_order(&mut self) -> Result<spirv::ImageChannelOrder> {
        if let Ok(word) = self.word() {
            spirv::ImageChannelOrder::from_u32(word)
                .ok_or(Error::ImageChannelOrderUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn image_channel_data_type(&mut self) -> Result<spirv::ImageChannelDataType> {
        if let Ok(word) = self.word() {
            spirv::ImageChannelDataType::from_u32(word)
                .ok_or(Error::ImageChannelDataTypeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn fp_rounding_mode(&mut self) -> Result<spirv::FPRoundingMode> {
        if let Ok(word) = self.word() {
            spirv::FPRoundingMode::from_u32(word)
                .ok_or(Error::FPRoundingModeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn fp_denorm_mode(&mut self) -> Result<spirv::FPDenormMode> {
        if let Ok(word) = self.word() {
            spirv::FPDenormMode::from_u32(word).ok_or(Error::FPDenormModeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn quantization_modes(&mut self) -> Result<spirv::QuantizationModes> {
        if let Ok(word) = self.word() {
            spirv::QuantizationModes::from_u32(word)
                .ok_or(Error::QuantizationModesUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn fp_operation_mode(&mut self) -> Result<spirv::FPOperationMode> {
        if let Ok(word) = self.word() {
            spirv::FPOperationMode::from_u32(word)
                .ok_or(Error::FPOperationModeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn overflow_modes(&mut self) -> Result<spirv::OverflowModes> {
        if let Ok(word) = self.word() {
            spirv::OverflowModes::from_u32(word).ok_or(Error::OverflowModesUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn access_qualifier(&mut self) -> Result<spirv::AccessQualifier> {
        if let Ok(word) = self.word() {
            spirv::AccessQualifier::from_u32(word)
                .ok_or(Error::AccessQualifierUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn host_access_qualifier(&mut self) -> Result<spirv::HostAccessQualifier> {
        if let Ok(word) = self.word() {
            spirv::HostAccessQualifier::from_u32(word)
                .ok_or(Error::HostAccessQualifierUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn function_parameter_attribute(&mut self) -> Result<spirv::FunctionParameterAttribute> {
        if let Ok(word) = self.word() {
            spirv::FunctionParameterAttribute::from_u32(word).ok_or(Error::FunctionParameterAttributeUnknown(
    ...
    pub fn group_operation(&mut self) -> Result<spirv::GroupOperation> {
        if let Ok(word) = self.word() {
            spirv::GroupOperation::from_u32(word)
                .ok_or(Error::GroupOperationUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn kernel_enqueue_flags(&mut self) -> Result<spirv::KernelEnqueueFlags> {
        if let Ok(word) = self.word() {
            spirv::KernelEnqueueFlags::from_u32(word)
                .ok_or(Error::KernelEnqueueFlagsUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn ray_query_intersection(&mut self) -> Result<spirv::RayQueryIntersection> {
        if let Ok(word) = self.word() {
            spirv::RayQueryIntersection::from_u32(word)
                .ok_or(Error::RayQueryIntersectionUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    #[doc = "Decodes and returns the next SPIR-V word as\na SPIR-V RayQueryCommittedIntersectionType value."]
    pub fn ray_query_committed_intersection_type(&mut self) -> Result<spirv::RayQueryCommittedIntersectionType> {
        if let Ok(word) = self.word() {
    ...
    pub fn ray_query_candidate_intersection_type(&mut self) -> Result<spirv::RayQueryCandidateIntersectionType> {
        if let Ok(word) = self.word() {
    ...
    pub fn packed_vector_format(&mut self) -> Result<spirv::PackedVectorFormat> {
        if let Ok(word) = self.word() {
            spirv::PackedVectorFormat::from_u32(word)
                .ok_or(Error::PackedVectorFormatUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn cooperative_matrix_operands(&mut self) -> Result<spirv::CooperativeMatrixOperands> {
        if let Ok(word) = self.word() {
            spirv::CooperativeMatrixOperands::from_bits(word).ok_or(Error::CooperativeMatrixOperandsUnknown(
                self.offset - WORD_NUM_BYTES,
                word,
            ))
        } else {
    ...
    pub fn cooperative_matrix_layout(&mut self) -> Result<spirv::CooperativeMatrixLayout> {
        if let Ok(word) = self.word() {
            spirv::CooperativeMatrixLayout::from_u32(word).ok_or(Error::CooperativeMatrixLayoutUnknown(
                self.offset - WORD_NUM_BYTES,
                word,
            ))
        } else {
    ...
    pub fn cooperative_matrix_use(&mut self) -> Result<spirv::CooperativeMatrixUse> {
        if let Ok(word) = self.word() {
            spirv::CooperativeMatrixUse::from_u32(word)
                .ok_or(Error::CooperativeMatrixUseUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn cooperative_matrix_reduce(&mut self) -> Result<spirv::CooperativeMatrixReduce> {
        if let Ok(word) = self.word() {
            spirv::CooperativeMatrixReduce::from_bits(word).ok_or(Error::CooperativeMatrixReduceUnknown(
                self.offset - WORD_NUM_BYTES,
                word,
            ))
        } else {
    ...
    pub fn tensor_clamp_mode(&mut self) -> Result<spirv::TensorClampMode> {
        if let Ok(word) = self.word() {
            spirv::TensorClampMode::from_u32(word)
                .ok_or(Error::TensorClampModeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn tensor_addressing_operands(&mut self) -> Result<spirv::TensorAddressingOperands> {
        if let Ok(word) = self.word() {
            spirv::TensorAddressingOperands::from_bits(word).ok_or(Error::TensorAddressingOperandsUnknown(
                self.offset - WORD_NUM_BYTES,
                word,
            ))
        } else {
    ...
    pub fn initialization_mode_qualifier(&mut self) -> Result<spirv::InitializationModeQualifier> {
        if let Ok(word) = self.word() {
            spirv::InitializationModeQualifier::from_u32(word).ok_or(Error::InitializationModeQualifierUnknown(
                self.offset - WORD_NUM_BYTES,
                word,
            ))
        } else {
    ...
    pub fn load_cache_control(&mut self) -> Result<spirv::LoadCacheControl> {
        if let Ok(word) = self.word() {
            spirv::LoadCacheControl::from_u32(word)
                .ok_or(Error::LoadCacheControlUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn store_cache_control(&mut self) -> Result<spirv::StoreCacheControl> {
        if let Ok(word) = self.word() {
            spirv::StoreCacheControl::from_u32(word)
                .ok_or(Error::StoreCacheControlUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    #[doc = "Decodes and returns the next SPIR-V word as\na SPIR-V NamedMaximumNumberOfRegisters value."]
    pub fn named_maximum_number_of_registers(&mut self) -> Result<spirv::NamedMaximumNumberOfRegisters> {
        if let Ok(word) = self.word() {
            spirv::NamedMaximumNumberOfRegisters::from_u32(word).ok_or(Error::NamedMaximumNumberOfRegistersUnknown(
                self.offset - WORD_NUM_BYTES,
                word,
            ))
        } else {
    ...
    pub fn matrix_multiply_accumulate_operands(&mut self) -> Result<spirv::MatrixMultiplyAccumulateOperands> {
        if let Ok(word) = self.word() {
    ...
    pub fn fp_encoding(&mut self) -> Result<spirv::FPEncoding> {
        if let Ok(word) = self.word() {
            spirv::FPEncoding::from_u32(word).ok_or(Error::FPEncodingUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    #[doc = "Decodes and returns the next SPIR-V word as\na SPIR-V CooperativeVectorMatrixLayout value."]
    pub fn cooperative_vector_matrix_layout(&mut self) -> Result<spirv::CooperativeVectorMatrixLayout> {
        if let Ok(word) = self.word() {
            spirv::CooperativeVectorMatrixLayout::from_u32(word).ok_or(Error::CooperativeVectorMatrixLayoutUnknown(
                self.offset - WORD_NUM_BYTES,
                word,
            ))
        } else {
    ...
    pub fn component_type(&mut self) -> Result<spirv::ComponentType> {
        if let Ok(word) = self.word() {
            spirv::ComponentType::from_u32(word).ok_or(Error::ComponentTypeUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn gather_modes(&mut self) -> Result<spirv::GatherModes> {
        if let Ok(word) = self.word() {
            spirv::GatherModes::from_u32(word).ok_or(Error::GatherModesUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
    ...
    pub fn tensor_operands(&mut self) -> Result<spirv::TensorOperands> {
        if let Ok(word) = self.word() {
            spirv::TensorOperands::from_bits(word)
                .ok_or(Error::TensorOperandsUnknown(self.offset - WORD_NUM_BYTES, word))
        } else {
            Err(Error::StreamExpected(self.offset))
        }
    }
