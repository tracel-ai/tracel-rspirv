...
pub fn gen_operand_decode_errors(grammar: &[structs::OperandKind]) -> TokenStream {
    let kinds: Vec<&str> = grammar
        .iter()
        .filter(|element| {
            !(element.kind.starts_with("Pair") || element.kind.starts_with("Id") || element.kind.starts_with("Literal"))
        })
...
pub fn gen_operand_decode_methods(grammar: &[structs::OperandKind]) -> TokenStream {
    let methods = grammar
        ...
        .map(|element| {
            ...
            let convert = as_ident(&format!(
                "from_{}",
                if element.category == structs::Category::BitEnum {
                    "bits"
                } else {
                    "u32"
                }
...
fn gen_operand_param_parse_methods(grammar: &[structs::OperandKind]) -> Vec<(&str, TokenStream)> {
    grammar
        ...
        .filter_map(|element| {
            ...
            let pairs: Vec<(&str, Vec<&str>)> = element
                .enumerants
                .iter()
                .filter_map(|e| {
                    let params: Vec<&str> = e.parameters.iter().map(|p| p.kind.as_str()).collect();
                    if params.is_empty() {
